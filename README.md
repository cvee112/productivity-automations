...
This is a collection of guides for app-free (script-based) productivity automations in macOS.
...
This schedules your calendar/task list/whatever URL to open in full screen mode via your default browser. It involves (1) [writing a bash script](#bash-1) that opens the relevant URL then (2) [scheduling a cron job](#cron-2) that runs the script every X time.
...
4. **(If you're using vim)** Press `i` for insert mode, then input something like:
    ...
    When run, this opens GCal in day view in full screen via my default browser, which is Arc.
